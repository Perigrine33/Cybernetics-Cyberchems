...
public final class AddictedVillagerProfessionHooks {
    ...
    private static final boolean MCA_LOADED =
            ModList.get().isLoaded("mca");
    ...
    public static void onVillagerTick(EntityTickEvent.Post event) {
        ...
        if (!MCA_LOADED) {
            convertToNitwit(villager);
        }

        villager.setCanPickUpLoot(true);
    }

    private static void convertToNitwit(Villager villager) {
        VillagerData villagerData =
                villager.getVillagerData();

        if (villagerData.getProfession()
